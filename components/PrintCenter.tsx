...
const PrintCenter: React.FC<PrintCenterProps> = ({ vouchers, activeCompany }) => {
  ...
  const applyFit = () => {
    if (containerRef.current) {
      ...
      setScale(Math.min(finalScale, 2.0));
      setIsAutoFit(true);
    }
  };

  // Trigger fit when document settings or mode changes
  useEffect(() => { 
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-5 gap-10 animate-in fade-in duration-500 pb-20 h-[calc(100vh-140px)]">
      ...
      <div className="xl:col-span-2 space-y-8 flex flex-col min-h-0">
        <div className="bg-white rounded-[3.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col flex-1 min-h-[300px]">
          <div className="px-10 py-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
             <h3 className="text-xl font-black italic uppercase text-slate-800 tracking-tight leading-none">Print Spooler</h3>
             <div className="flex items-center space-x-3">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{selectedVchIds.length} Linked</span>
        ...
        <div className="bg-slate-900 rounded-[3rem] p-10 text-white relative overflow-hidden shadow-2xl border-t-8 border-indigo-500 shrink-0">
           <h3 className="text-[11px] font-black uppercase text-indigo-400 tracking-[0.4em] mb-10">Advanced Configurator</h3>
           <div className="space-y-8">
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Media Size</label>
                ...
                <div className="space-y-2">
                  ...
                  <select value={orientation} onChange={e => setOrientation(e.target.value as Orientation)} className="w-full bg-white/5 border border-white/10 rounded-[1.5rem] px-5 py-4 text-xs font-black text-white outline-none cursor-pointer hover:bg-white/10 transition-colors appearance-none">
                    <option value="Portrait" className="bg-slate-900">Portrait View</option>
                    <option value="Landscape" className="bg-slate-900">Landscape View</option>
                  </select>
                </div>
                <div className="space-y-2">
      ...
      {/* Right Content: Advanced Preview Node */}
         ...
         <div className="bg-white border border-slate-200 rounded-t-[4rem] p-6 flex flex-col sm:flex-row items-center justify-between shadow-lg z-10 mx-6 mb-[-1px] relative shrink-0 gap-4">
            <div className="flex items-center space-x-4">
               <div className="px-6 py-2.5 bg-slate-900 text-white rounded-full flex items-center space-x-4 shadow-xl border border-slate-800">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.3em] italic">Pre-Flight Inspector</span>
               </div>
            </div>
            ...
            <div className="flex items-center space-x-4 flex-1 justify-end">
               ...
               <button onClick={() => handleScaleChange(1.0)} className="px-3 py-2 bg-white border border-slate-200 rounded-xl text-[9px] font-black text-slate-500 uppercase hover:border-indigo-200 hover:text-indigo-600 transition-all shadow-sm">100%</button>
            </div>
         </div>

         {/* Scaled Document Container */}
         <div ref={containerRef} className="bg-slate-800 rounded-b-[4rem] border-4 border-slate-900 flex-1 overflow-auto custom-scrollbar shadow-2xl relative scroll-smooth bg-[radial-gradient(circle_at_center,_var(--tw-gradient-from)_0%,_var(--tw-gradient-to)_100%)] from-slate-800 to-slate-900">
            <div className="flex justify-center min-h-full p-20 pt-10">
               {previewVch ? (
                 ...
                 <div className="flex flex-col items-center justify-center h-full text-slate-500 space-y-6 animate-pulse mt-40">
                    ...
                    <p className="text-sm font-black uppercase tracking-[0.3em] italic opacity-40 text-white">Authorize Vouchers for Inspection</p>
                 </div>
               )}
            </div>
