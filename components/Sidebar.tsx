...
const SubMenuItem: React.FC<{ label: string, id: string, activeId: string | null, onClick: () => void }> = ({ label, id, activeId, onClick }) => {
  ...
  return (
    <button 
      onClick={onClick} 
      className={`w-full text-left px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-[0.1em] transition-all relative group/sub flex items-center ${
        isActive 
          ? 'text-white bg-indigo-500/20 ring-1 ring-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.1)]' 
          : 'text-slate-400 hover:text-slate-200 hover:bg-white/5'
      }`}
    >
      <div className={`w-1.5 h-1.5 rounded-full mr-3 transition-all duration-300 ${
        isActive ? 'bg-indigo-400 scale-100 shadow-[0_0_8px_#818cf8]' : 'bg-slate-700 scale-75 opacity-40 group-hover/sub:bg-slate-500'
      }`} />
      <span className="truncate block flex-1">{label}</span>
      {isActive && (
        <svg className="w-3 h-3 text-indigo-400 animate-in fade-in zoom-in duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const SubMenuContainer: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="ml-4 mt-1 mb-3 space-y-1 border-l-2 border-slate-800/60 pl-3 animate-in fade-in slide-in-from-left-2 duration-300">
    {children}
  </div>
);
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-950 text-white transition-all duration-500 ease-in-out transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 border-r border-slate-900 flex flex-col h-screen overflow-hidden shadow-2xl`}>
      ...
      {/* Navigation Tree */}
      <nav className="flex-1 mt-6 px-3 space-y-1.5 overflow-y-auto custom-scrollbar pb-8 scroll-smooth">
        {MENU_ITEMS.map((item) => {
          ...
          return (
            <div key={item.id} className="space-y-1">
              <button
                ...
                className={`w-full flex items-center space-x-3 px-4 py-3 rounded-2xl transition-all duration-300 relative group overflow-hidden ${
                  isParentActive 
                    ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-950/80 ring-1 ring-indigo-400/30' 
                    : 'text-slate-500 hover:bg-white/5 hover:text-slate-100'
                }`}
              >
                {/* Active Indicator Bar */}
                {isParentActive && (
                  <div className="absolute left-0 top-3 bottom-3 w-1 bg-white rounded-r-full shadow-[0_0_8px_#fff] animate-in slide-in-from-left duration-300" />
                )}
                
                <span className={`${isParentActive ? 'text-white scale-110' : 'text-slate-600 group-hover:text-indigo-400'} transition-all duration-300 w-6 h-6 flex items-center justify-center shrink-0`}>
                  {React.cloneElement(item.icon as React.ReactElement, { className: 'w-5 h-5' })}
                </span>
                
                <span className={`font-black text-[11px] uppercase tracking-[0.15em] flex-1 text-left truncate italic ${isParentActive ? 'text-white' : ''}`}>
                  {item.label}
                </span>

                {hasSub && (
                  <div className={`transition-all duration-500 ${isParentActive ? 'rotate-180 scale-110' : 'text-slate-700 group-hover:text-slate-400'}`}>
                ...
                {isParentActive && (
                  <div className="absolute inset-0 bg-gradient-to-r from-white/10 to-transparent pointer-events-none opacity-50" />
                )}
              </button>
      ...
      <div className="p-4 border-t border-slate-900 bg-slate-950 shrink-0">
        <div className="flex items-center space-x-3 p-3 bg-slate-900/50 rounded-2xl border border-slate-800/50 hover:bg-slate-900 transition-colors cursor-pointer group">
