...
import { Voucher, Item, Ledger, AuditLog } from '../types';
...
const ImportExportModule: React.FC<ImportExportModuleProps> = ({ 
  ...
  const LEDGER_IMPORT_FIELDS: ImportField[] = [
    { key: 'name', label: 'Ledger Title', required: true },
    { key: 'group', label: 'Account Group', required: true },
    { key: 'openingBalance', label: 'Opening Balance', type: 'number', defaultValue: 0 },
    { key: 'type', label: 'Balance Type (Debit/Credit)', defaultValue: 'Debit' }
  ];

  const handleBulkLedgerImport = (importedData: any[]) => {
    if (setLedgers) {
      setLedgers(prev => [...prev, ...importedData]);
      if (addAuditLog) {
        addAuditLog({
          action: 'CREATE',
          entityType: 'MASTER',
          entityName: 'LEDGER_BULK_IMPORT',
          details: `Master Registry Sync: ${importedData.length} ledger nodes integrated via bulk ingestion.`
        });
      }
      alert(`Ledger Import Success: ${importedData.length} records synthesized.`);
    }
  ...
  const executeExport = () => {
    ...
    else if (selectedEntity === 'Ledger Masters') data = ledgers;
  ...
  return (
    <div className="bg-white rounded-[4rem] border border-slate-200 p-16 shadow-sm animate-in fade-in duration-500 max-w-5xl mx-auto flex flex-col relative">
      <div className="flex items-center justify-between mb-16">
        ...
        <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200 shadow-inner shrink-0">
           {(['IMPORT', 'EXPORT'] as const).map(t => (
             <button key={t} onClick={() => setActiveTab(t)} className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === t ? 'bg-white text-indigo-600 shadow-lg scale-105' : 'text-slate-500'}`}>{t}</button>
           ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 flex-1">
        ...
        <div className="flex flex-col h-full">
           <div 
             ...
             onClick={activeTab === 'EXPORT' ? executeExport : () => setIsImportWizardOpen(true)}
           >
              ...
              <p className="text-[10px] font-bold text-slate-400 mt-4 uppercase tracking-[0.4em]">{activeTab === 'EXPORT' ? `Execute ${targetFormat} transmission for ${selectedEntity}` : `Ingest ${selectedEntity} shards into core`}</p>
           </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-300">
           <div className="w-full max-w-4xl max-h-[90vh] overflow-y-auto custom-scrollbar shadow-2xl">
              <CsvImportWizard 
                ...
                fields={selectedEntity === 'Ledger Masters' ? LEDGER_IMPORT_FIELDS : (selectedEntity === 'Inventory Items' ? [
                  { key: 'name', label: 'Item Designation', required: true },
                  { key: 'category', label: 'Category', defaultValue: 'General' },
                  { key: 'unit', label: 'Unit', defaultValue: 'Nos' },
                  { key: 'salePrice', label: 'Price', type: 'number' }
                ] : [])}
                existingKeys={selectedEntity === 'Ledger Masters' ? ledgers.map(l => l.name) : (selectedEntity === 'Inventory Items' ? items.map(i => i.name) : [])}
                onCancel={() => setIsImportWizardOpen(false)} 
                onImport={selectedEntity === 'Ledger Masters' ? handleBulkLedgerImport : (selectedEntity === 'Inventory Items' ? (data) => { setItems && setItems(prev => [...prev, ...data]); setIsImportWizardOpen(false); } : () => {})} 
              />
           </div>
        </div>
      )}
