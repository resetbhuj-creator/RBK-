...
const TaxGroupForm: React.FC<TaxGroupFormProps> = ({ initialData, taxes = [], onCancel, onSubmit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (initialData?.isSystem) {
      onCancel(); // System records are view-only or handled differently
      return;
    }
    if (validate()) {
      onSubmit(formData);
    ...
    <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-300 max-w-2xl mx-auto">
      <div className="px-10 py-8 bg-slate-900 text-white flex justify-between items-center relative overflow-hidden">
        <div className="relative z-10">
          <h3 className="text-xl font-black italic uppercase tracking-tight">{isSystem ? 'System Tax Umbrella' : (initialData ? 'Modify Tax Umbrella' : 'Provision Tax Group')}</h3>
      ...
      <form onSubmit={handleSubmit} className="p-10 space-y-8">
        <div className="space-y-1">
          ...
          <input 
            ...
            className={`w-full px-6 py-4 rounded-2xl border outline-none transition-all text-sm font-bold ${isSystem ? 'bg-slate-50 text-slate-500 cursor-not-allowed' : (errors.name ? 'border-rose-500 bg-rose-50/30' : 'border-slate-200 focus:ring-4 focus:ring-indigo-500/10 bg-white shadow-sm')}`}
          />
          {errors.name && !isSystem && <p className="text-[10px] text-rose-500 font-black mt-2 ml-1">{errors.name}</p>}
          {isSystem && <p className="text-[9px] text-amber-500 font-bold mt-2 ml-1 italic">Core system identifiers are cryptographically locked.</p>}
        </div>
        ...
        <div className="space-y-1">
          ...
          <textarea 
            ...
            className={`w-full px-6 py-4 rounded-2xl border border-slate-200 bg-slate-50 text-sm font-medium italic shadow-inner outline-none focus:ring-4 focus:ring-indigo-500/10 ${isSystem ? 'cursor-not-allowed' : 'bg-white shadow-sm'} resize-none leading-relaxed`}
          />
        </div>
        ...
        <div className="pt-8 border-t border-slate-100 flex justify-end space-x-4">
          <button type="button" onClick={onCancel} className="px-10 py-4 rounded-2xl text-slate-400 font-black text-[10px] uppercase tracking-widest hover:bg-slate-50 transition-all">
