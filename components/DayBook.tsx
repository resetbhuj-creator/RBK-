...
const DayBook: React.FC<DayBookProps> = ({ vouchers, onClone }) => {
  ...
  const filtered = useMemo(() => {
    return vouchers.filter(v => {
      ...
      if (datePreset === 'Today') matchesDate = vDate.toDateString() === today.toDateString();
      if (datePreset === 'Week') {
        const lastWeek = new Date();
        lastWeek.setDate(today.getDate() - 7);
        matchesDate = vDate >= lastWeek;
      }
      if (datePreset === 'Month') matchesDate = vDate.getMonth() === today.getMonth() && vDate.getFullYear() === today.getFullYear();

      return matchesSearch && matchesType && matchesMin && matchesMax && matchesDate;
    });
  }, [vouchers, searchTerm, filterType, minAmount, maxAmount, datePreset]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden">
        ...
        {showAdvanced && (
           <div className="p-8 bg-white border-b border-slate-100 grid grid-cols-1 md:grid-cols-3 gap-10 animate-in slide-in-from-top-4 duration-500">
              ...
              <div className="flex items-end">
                 <button onClick={() => { setMinAmount(''); setMaxAmount(''); setDatePreset('All'); setSearchTerm(''); }} className="w-full py-4 text-[10px] font-black uppercase tracking-widest text-rose-500 hover:bg-rose-50 rounded-2xl transition-all">Reset Intelligent Filters</button>
              </div>
           </div>
        )}
              ...
              {filtered.map((v) => (
                <tr key={v.id} className="hover:bg-slate-50/80 transition-colors group">
                  <td className="px-10 py-6">
                    <span className="font-black text-slate-800 text-sm tracking-tight italic group-hover:text-indigo-600 transition-colors">{v.id}</span>
        ...
        {filtered.length === 0 && (
           <div className="py-32 text-center space-y-6">
              ...
              <p className="text-sm font-black uppercase text-slate-300 tracking-[0.2em] italic">Zero transaction footprints found for current selection</p>
           </div>
        )}
      </div>
    </div>
